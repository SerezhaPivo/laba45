...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        BasicStroke dashed = new BasicStroke(3, BasicStroke.CAP_BUTT,
                ...
                0, new float[]{4, 4, 8, 4, 4, 4, 32, 4, 8, 4}, 0);
    ...
    public void paintComponent(Graphics g) {
        ...
        if(ShowCriticalValues) paintRound(canvas);
    ...
    private void paintRound(Graphics2D canvas){
        ...
        for (Double[] point : graphicsData) {
            if (point[0] >= viewport[0][0] && point[1] <= viewport[0][1] && point[0] <= viewport[1][0]) {
                ...
                if (currentX != null && currentY != null) {
                    canvas.draw(new Line2D.Double(xyToPoint(currentX, currentY), xyToPoint(point[0], Math.abs(point[1]))));
                }
                currentX = point[0];
                currentY = Double.valueOf(Math.abs(point[1]));
            }
        }
        if(firstlunc){
            zoomToRegion(minX, maxY, maxX, minY);
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
            ...
            if (point[0] >= viewport[0][0] && point[1] <= viewport[0][1] && point[0] <= viewport[1][0]) {
                ...
                boolean sign = false;
                String s = String.valueOf(Math.round(point[1]));
                for(int i = 0; i < 20; i++){
                    if(Math.pow(Integer.valueOf(s), 0.5) == i){
                        sign = true;
                    }
                }
                ...
                if (i_n == this.selectedMarker) {
                    ...
                    canvas.draw(new Line2D.Double(center.x - 5.5, center.y - 5.5, center.x, center.y + 5.5));
                    canvas.draw(new Line2D.Double(center.x, center.y + 5.5, center.x + 5.5, center.y - 5.5));
                    //canvas.draw(new Line2D.Double(center.x + 5.5, center.y - 5.5, center.x, center.y - 5.5));
                    //canvas.draw(new Line2D.Double(center.x, center.y - 5.5, center.x - 5.5, center.y - 5.5));
